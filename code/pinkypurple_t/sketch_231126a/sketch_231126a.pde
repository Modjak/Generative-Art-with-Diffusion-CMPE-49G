class Line {
  ...
  Line(PVector pos, color lineColor) {
    this.pos = pos;
    vel = new PVector(0, 0);
    acc = new PVector(0, 0);
 ...
 void proceed(Field fa, Field fb, float perlinOffset) {
  futPos();
  follow(fa);
  fb.depo(pos, vel, perlinOffset);
  spin();
  update();
  edge();

  float noiseVal = noise(pos.x * 0.01, pos.y * 0.01, perlinOffset);
  int alpha = int(map(noiseVal, 0, 1, 20, 100)); 
  stroke(map(noiseVal, 0, 1, 0, 255), map(noiseVal, 0, 1, 50, 150), map(noiseVal, 0, 1, 100, 255), alpha);
...
class Field {
  ...
  void depo(PVector loc, PVector ph, float perlinOffset) {
    int cc = int(constrain(loc.x / index, 0, cols - 1));
    int rr = int(constrain(loc.y / index, 0, rows - 1));
    PVector tmp = ph.get();
    tmp.normalize();
    tmp.rotate(map(noise(loc.x * 0.01, loc.y * 0.01, perlinOffset), 0, 1, -PI, PI)); 
    ff[cc][rr] = tmp;
  }
...
void setup() {
  ...
  for (int i = 0; i < num; i++) {
    ...
    Line a = new Line(pos, color(0, 0, 255)); 
    lines.add(a);
  }
  for (int i = 0; i < num1; i++) {
    PVector pos1 = new PVector(random(width), random(height));
    Line a1 = new Line(pos1, color(255, 255, 0)); 
    lines1.add(a1);
  }
  background(0);
}
